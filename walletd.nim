...
proc initWallet*(host="127.0.0.1", port="8070", pwd: string): Wallet =
...
#[
    getUnconfirmedTransactionHashes - returns information about the current unconfirmed transaction pool or for a specified addresses
]#
...
method getTransaction*(w: Wallet, txHash: string): JsonNode {.base.} =
    let params = %*{"transactionHash": txHash}
...
#[
    deleteDelayedTransaction - deletes a specified delayed transaction
]#
method deleteDelayedTransaction*(w: Wallet, txHash: string): JsonNode {.base.} =
    let params = %*{"transactionHash": txHash}
...
method sendDelayedTransaction*(w: Wallet, txHash: string): JsonNode {.base.} =
    let params = %*{"transactionHash": txHash}
